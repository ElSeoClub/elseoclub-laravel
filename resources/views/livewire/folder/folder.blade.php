<div class="max-w-6xl mx-auto py-12 space-y-8">
    ...
    <x-card class="{{$folder->with_files && $folder->with_subfolders ? 'col-span-4' : 'col-span-12'}}">
                ...
                @if($folders->count())
                    @foreach($folders as $f)
                        <tr class=" hover:bg-gray-50 cursor-pointer">
                            <td>
                                <a href="{{route('empresas.folder.index', $f)}}" class="p-2 block w-full">{{$f->name}}</a>
                            </td>
                        </tr>
                    @endforeach
                @else
        ...
        <x-slot name="footer">
            <x-button color="blue" icon="fas fa-plus" class="w-full lg:max-w-xs" :href="route('empresas.folder.create',$folder)">Agregar</x-button>
        </x-slot>
    </x-card>
@endif
    ...
    <x-card class="{{$folder->with_files && $folder->with_subfolders ? 'col-span-8' : 'col-span-12'}}">
            ...
            <table class="table-auto w-full dark:text-gray-300">
                ...
                @if($files->count())
                    @foreach($files as $f)
                        <tr class=" hover:bg-gray-50 cursor-pointer">
                            <td>
                                <a href="/storage/{{$f->path}}" target="_blank" class="p-2 block w-full">{{$f->name}}</a>
                            </td>
                        </tr>
                    @endforeach
                @else
        ...
        <x-slot name="footer">
            <x-button color="blue" icon="fas fa-plus" class="w-full max-w-xs" :href="route('empresas.folder.file',$folder)">Agregar</x-button>
        </x-slot>
    </x-card>
@endif
    </div>
