<div>
        ...
        <div class="flex" style="height:calc(100vh - 310px)">
            <div class="w-3/4 flex p-3">
                ...
                <div class="w-2/3 ml-6">
                    <div class="w-full text-center text-xl mb-3">Viernes 9 de Julio de 2021 - <span id="clock"></span>
                    </div>
                            ...
                            <div class="-m-5 px-3">
                                <div class="relative z-0 w-full mb-5">
                                    <input type="text" placeholder=" " autocomplete="off"
                                        ...
                                        wire:keydown.enter="get_user($event.target.value)" wire:model.defer="qr_code" />
                    ...
                    <div class="rounded bg-white shadow-xl mt-3">
                        ...
                        <div class="flex p-3">
                            @foreach ($this->sede->assistants()->paginate(6) as $last)
            ...
            <div class="w-1/4 pl-3 py-3">
                <x-card title="Encargados de la sede">
                    <div class="-m-5" style="height:calc(100vh - 393px); overflow-y:auto">
                        @php
                        $n = 0;
                        @endphp
                        @foreach($sede->users()->get() as $user)
    ...
    <div class="mt-6" style="height:130px">
        <div class="w-full h-full">
            <span class="font-bold text-xl">Asistencia al centro de votación:
                {{$avance['asistencia_total']}} trabajadores, de los cuales
                {{$avance['asistencia_foranea']}}
                fue foraneo.
                Avance en el padrón local:
                {{$avance['avance_padron']}} de {{$avance['total_padron']}}
                ...
                <div class="shadow-md w-full bg-gray-200 h-9 mb-3 flex">
                    <div class="bg-green-500 text-xs leading-none text-center text-white h-full py-2 font-bold text-lg z-10"
                        style="width: {{$avance['porcentaje_local']}}%">
                        {{$avance['porcentaje_total']}}%
                    </div>
                    <div class="bg-green-700 text-xs leading-none text-center text-white h-full py-2 font-bold text-lg"
                        style="width: {{$avance['porcentaje_foraneo']}}%">
                    </div>
                </div>
                <span class="font-bold text-xl">Asistencia global de votación:
                    {{$avance['avance_global']}} de {{$avance['padron_global']}}</span>
                <div class=" shadow-md w-full bg-gray-200 h-9 rounded">
                    <div class="bg-green-500 text-xs leading-none text-center text-white h-full rounded py-2 font-bold text-lg"
                        style="width: {{$avance['porcentaje_global']}}%">
                        {{$avance['porcentaje_global']}}%
                    </div>
                </div>
        </div>
    </div>
